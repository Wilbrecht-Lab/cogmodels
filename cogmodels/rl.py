...
class RL_st(RL_4p):
    ...
    def id_param_init(self, params, param_id):
        ...
        fixp_list = ["b0", "q_init", "gam"]
        alpha, beta, st = params.loc[params["ID"] == param_id, varp_list].values.ravel()
        b0, q0, gam = [self.fixed_params[fp] for fp in fixp_list]
        w0 = np.array([q0] * 2, dtype=float)
...
class RL(RL_4p):
    ...
    def id_param_init(self, params, param_id):
        ...
        fixp_list = ["b0", "q_init", "gam"]
        alpha, beta = params.loc[params["ID"] == param_id, varp_list].values.ravel()
        b0, q0, gam = [self.fixed_params[fp] for fp in fixp_list]
        w0 = np.array([q0] * 2, dtype=float)
...
class RL_Forgetting(RL_4p):
    ...
    def id_param_init(self, params, param_id):
        ...
        var_dict = params.loc[params["ID"] == param_id, varp_list].iloc[0].to_dict()
